...
public class PdsListCommand implements PdsInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int pageSize = request.getParameter("pageSize")==null || request.getParameter("pageSize").equals("") ? 5 : Integer.parseInt(request.getParameter("pageSize"));//확장성 고려차원에서 이렇게 해둠 원래는 그냥 5해도된다
		
		//3. 총 레코드 건수를 구한다 (totRecCnt) - sql 명령어 중에서 count함수 이용
		int totRecCnt = dao.getTotRecCnt(part); // 여기서 레벨이 가야 된다
